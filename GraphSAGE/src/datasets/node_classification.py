...
class Cora(Dataset):
    ...
    def __init__(self, path, mode, num_layers,
        ...
        cites = np.loadtxt(os.path.join(path, 'cora.cites'), dtype=np.int64)
        cont = np.loadtxt(os.path.join(path, 'cora.content'), dtype=str)
        print('Finished reading data.')

        print('Setting up data structures.')
        if transductive:
            idx = np.arange(cont.shape[0])
        else:
            if mode == 'train':
                idx = self.idx['train']
            elif mode == 'val':
        ...
        features, labels = cont[idx, 1:-1].astype(np.float32), cont[idx, -1]
        d = {j : i for (i,j) in enumerate(sorted(set(labels)))}
        labels = np.array([d[l] for l in labels])

        vertices = np.array(cont[idx, 0], dtype=np.int64)
        d = {j : i for (i,j) in enumerate(vertices)}
        edges = np.array([e for e in cites if e[0] in d.keys() and e[1] in d.keys()])
